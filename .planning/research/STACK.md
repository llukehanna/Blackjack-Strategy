...
Rationale: iOS 17 is the floor for SwiftData and `@Observable`, but iOS 18 gives us improved SwiftUI scroll views, better animation APIs, and aligns with App Store SDK requirements (iOS 18 SDK minimum since early 2025). By the time this ships, iOS 18 adoption will be well above 90%. There is no reason to support iOS 16/17 for a greenfield consumer app launching in mid-2026.

**Confidence: HIGH** -- These are Apple's current shipping tools, verified against Apple Developer documentation and release notes.
...
**Use `@Observable` classes as ViewModels.** This is the modern SwiftUI pattern as of 2025-2026. The old `ObservableObject` + `@Published` + Combine pipeline approach is legacy. `@Observable` (Observation framework, iOS 17+) gives:
...
ViewModel Layer (@Observable classes)
    |
Domain Layer (pure Swift structs/classes -- rules engine, strategy tables, statistics)
    |
Persistence Layer (SwiftData models + UserDefaults)
```

**Confidence: HIGH** -- This is the consensus modern SwiftUI architecture per Apple's own migration guides and the broader iOS community in 2025-2026.
...
SwiftData underwent significant internal refactoring between iOS 17 and iOS 18, causing compatibility issues for early adopters. By targeting iOS 18+, we avoid the worst of these growing pains. Still, complex migrations and advanced queries (GROUP BY, NSFetchedResultsController equivalents) remain less mature than Core Data. For this app's data model (simple session records, aggregate stats), this is not a concern.

**Confidence: MEDIUM-HIGH** -- SwiftData is the clear direction for new projects but has a shorter track record than Core Data. The data model here is simple enough that risk is low.
...
**Confidence: HIGH** -- Swift Testing is Apple's current recommended framework, verified via Apple Developer docs. XCTest coexistence is explicitly supported.
...
**Confidence: HIGH** -- SPM is the uncontested standard for new Swift projects in 2026.

---

### Third-Party Dependencies

**Recommendation: Minimize or eliminate third-party dependencies.**

This app's domain (blackjack rules, card counting, statistics) is well-defined and self-contained. The primary value is correctness of the rules engine, which must be owned and thoroughly tested -- not delegated to a third-party library.
...
**Confidence: HIGH** -- The zero-dependency approach is deliberate. Every feature this app needs is covered by Apple's first-party frameworks.
...
No `Package.swift` dependencies needed for v1. If a dependency is added later, use SPM via Xcode's "Add Package Dependencies" workflow.
...
Swift 6.2's "Approachable Concurrency" (shipping with Xcode 26) is relevant for this project:
...
Practical impact: The rules engine and edge calculator should be designed as `Sendable` types from the start, enabling background computation without data race issues.
